...
def log_performance(test_name, metrics, output_dir, args_machine_name, paraview_version=None, visit_version=None):
    """
...
def create_baseline_images(output_dir, max_images=5):
    ...
    if not os.path.exists(output_images_dir):
        print(f"No output images found in {output_images_dir}. Skipping baseline creation.")
        return

    # List all images in the output directory
    image_files = sorted([f for f in os.listdir(output_images_dir) if f.endswith(('.png', '.jpg', '.jpeg'))])
...
def compare_text_files(output_log, known_good_value_path, ignore_patterns=None):
    ...
    known_good_content = [clean_content(line) for line in known_good_content if clean_content(line)]
...
def create_summary_report(test_directory, test_type, args_machine_name):
    ...
    subdirectories = sorted([d for d in os.listdir(test_directory) if d.startswith('ex') and os.path.isdir(os.path.join(test_directory, d))])
    ...
    for subdir in subdirectories:
        ...
        performance_file = os.path.join(testing_dir, f'performance_metrics_{machine_name}.json')
        if os.path.exists(performance_file):
            ...
            if significant_changes:
                ...
                summary_report['significant_performance_changes'].append({subdir: significant_changes})
                summary_report['any_tests_failed'] = True
        else:
...
def run_test(test_dir, dir_name, args):
    ...
    if submit:
        ...
        log_performance(dir_name, metrics, test_dir, args.machine_name, paraview_version=args.paraview_version, 
    ...
    comparison_results_file = os.path.join(test_dir, 'Testing', 'image_comparison_results.json')
    ...
    if os.path.exists(output_log_path):
        known_good_value_file = os.path.join(test_dir, 'Testing', 'Baseline', 'known_good_value.txt')  # Update this path as necessary
        if os.path.exists(known_good_value_file):
            text_comparison_result = compare_text_files(output_log_path, known_good_value_file)
            text_comparison_results_output = []
            text_comparison_results_output.append({'logs_match': text_comparison_result})

            # Save text comparison results
            text_comparison_results_file = os.path.join(test_dir, 'Testing', 'text_comparison_results.json')
...
def main():
    ...
    parser.add_argument('--submit', action='store_true', help="Submit tests to cluster instead of running locally.")
    parser.add_argument('--generate-metrics', action='store_true', help="Generate metrics and graphs without running tests.")
    parser.add_argument('--clean', action='store_true', help='Clean up generated test files')
    parser.add_argument('--test_number', type=int, help="Specify the test number to run (e.g., 0 for ex00, 1 for ex01, etc.).")
    parser.add_argument('--machine_name', type=str, help="Optional machine name to use in performance metrics")
    parser.add_argument('--paraview_version', type=str, default=None, help='Specify the ParaView version (e.g., 5.13.1)')
    parser.add_argument('--visit_version', type=str, default=None, help='Specify the VisIt version (e.g., 3.2.0)')

    args = parser.parse_args()

    test_directory = args.root_directory + args.test_type + "_Vignettes"
    example_dirs = [d for d in os.listdir(test_directory) if d.startswith('ex') and os.path.isdir(os.path.join(test_directory, d))]
    ...
    if args.test_number is not None:
        ...
        if args.test_number < len(example_dirs):
            ...
            print(f"Error: Test number {args.test_number} is out of range. Available tests: 0-{len(example_dirs)-1}")
