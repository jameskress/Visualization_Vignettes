...
def generate_individual_graphs(test_directory, current_sub_test):
    ...
    for file_name in performance_files:
        ...
        if performance_data:
            ...
            df = pd.DataFrame.from_dict(performance_data, orient='index')  # Create DataFrame from dictionary
            df.index = pd.to_datetime(df.index)  # Convert index (which is timestamp) to datetime
    ...
    for metric, ylabel in metrics.items():
        ...
        for i, machine_name in enumerate(sorted_machines):
            ...
            if metric in df.columns:
                ...
                for j in range(len(df)):
                    ...
                    marker_shape, returned_version = get_marker_shape(row_data, marker_map)  # Get the marker shape dynamically based on version
                    
                    # Plot the point with the selected marker
                    plt.plot(timestamp, value, marker=marker_shape, color=colors[i], markersize=8)
            ...
            if machine_name not in added_labels:
                plt.plot([], [], linestyle='-', linewidth=4, color=colors[i], label=machine_name)  # Add to legend
        ...
        main_legend = plt.legend(title="Machines", loc='upper left', bbox_to_anchor=(1, 1), frameon=False, fontsize=14)
        ...
        handles = [plt.Line2D([0], [0], marker=marker, linestyle='None', color='black', markersize=10) for marker in version_shapes.values()]
        version_labels = list(version_shapes.keys())  # Use only version names
        version_legend = plt.legend(handles, version_labels, title="Versions", loc='upper left', bbox_to_anchor=(1, 0.5), frameon=False, fontsize=14)
        version_legend.get_title().set_fontsize(14)  # Set version legend title font size

        # Add both legends to the plot
        plt.gca().add_artist(main_legend)  # Ensure main legend is added back to the plot
