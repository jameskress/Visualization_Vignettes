...
def gather_metrics(test_name, start_time, end_time):
    ...
    metrics = {
        ...
        'memory_usage_mb': (psutil.Process().memory_info().rss / (1024 * 1024)),  # Convert to MB
...
def detect_significant_changes(df, threshold=10):
    ...
    metrics = ['execution_time', 'memory_usage_mb', 'cpu_usage_percent', 'disk_usage_percent']
    ...
    for metric in metrics:
        if metric in df.columns:
            ...
            if len(last_two_runs) == 2:
                ...
                print(f"\tComparing {metric}: previous={previous_value}, current={current_value}")  # Debugging

                percent_change = 100 * (current_value - previous_value) / previous_value
                #print(f"percent change: {percent_change}")

                if percent_change > threshold:
                    print(f"\tSignificant change detected for {metric}: {percent_change}% change")
                    return {
